...
WHILE i < MAX_ITERATIONS AND NOT foundHash DO
	SET gameHash = CONCAT(
		substring(@chars, rand(@seed:=round(rand()*@intSize))*@charCount+1,1),
        substring(@chars, rand(@seed:=round(rand(@seed)*@intSize))*@charCount+1,1),
        substring(@chars, rand(@seed:=round(rand(@seed)*@intSize))*@charCount+1,1),
        substring(@chars, rand(@seed:=round(rand(@seed)*@intSize))*@charCount+1,1),
        substring(@chars, rand(@seed:=round(rand(@seed)*@intSize))*@charCount+1,1),
        substring(@chars, rand(@seed:=round(rand(@seed)*@intSize))*@charCount+1,1)
    );
    IF NOT EXISTS (SELECT hash FROM lobbies WHERE hash = gameHash) THEN
		SET foundHash = true;
    END IF;
    SET i = i + 1;
END WHILE;
